...
abstract class PDO implements \core\sgbd\SQL_interface {
	...
	public static function connect() {
		...
		try {
			self::$connection = new \PDO(
					\core\Configuracion::$db["dsn"], \core\Configuracion::$db["user"], \core\Configuracion::$db["password"]);
		}
	...
	public static function disconnect() {
		...
		// Destruir el objeto $connection o la conexión
	...
	public static function execute($sql) {
		...
		if ( self::$result === false) {
			if (self::$depuracion) {
				throw new \Exception(__METHOD__." Consulta= $sql <br />Error = ".implode(" - ",self::$connection->errorInfo()));
			}
			else {
	...
	public static function get_rows($sql = null) {
		...
		return (is_string($sql) and strlen($sql))  ? self::execute($sql) : self::$result->fetchAll(\PDO::FETCH_ASSOC);
...
//		return $filas[0]['id'];
	...
	public static function start_transaction() {
		...
		return (self::$connection->inTransaction() ? null : self::$connection->beginTransaction());
	...
	public static function get_error() {
		...
		return ((self::$result === false) ? implode(" - ",self::$connection->errorInfo()) : null);
